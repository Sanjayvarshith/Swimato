...
async (req,res)=>{
    ...
    try{
        ...
        if(!userData){
            return res.status(400).json({ errors: "Incorrect email/password1" });
        }
        ...
        if(!pwdCompare)
        {
            // console.log(`Password mismatch: ${req.body.password} !== ${userData.password}`);
            return res.status(400).json({ errors: "Incorrect email/password2" });   
        }
        const data={
            user:{
                id:userData.id
